...
def _su(path=""):
    """Studio URL + ép hl=en để DOM luôn tiếng Anh (selector nhất quán)."""
    base = studio.STUDIO_URL + path
    sep = "&" if "?" in base else "?"
    return f"{base}{sep}hl=en"
...
def _goto_content(session):
    ...
    if not cid:
        ...
        session.goto(_su(), wait_ms=8000, name="mgr-home")
        cid = _channel_id(session)
    r = session.goto(_su(f"/channel/{cid}{CONTENT_PATH}"),
...
def get_video(session, video_id):
    ...
    r = session.goto(_su(f"/video/{video_id}/edit"),
...
def delete_video(session, video_id, confirm=False):
    ...
    step2 = session.evaluate("""(()=>{
      ...
      const del=items.find(i=>/^\\s*(Delete|X[oóòỏ]a)\\s*$/i.test((i.textContent||'').trim()));
    ...
    step4 = session.evaluate("""(()=>{
      ...
      const d=btns.find(b=>/(delete forever|x[oóòỏ]a vĩnh viễn)/i.test((b.textContent||'').trim())&&!b.hasAttribute('disabled'));
