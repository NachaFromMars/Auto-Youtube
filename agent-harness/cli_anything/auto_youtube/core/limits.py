...
STATE_FILE = os.path.join(STATE_DIR, "rate_state.json")   # global (legacy/mặc định)

MAX_UPLOADS_PER_DAY = 50          # HARD CAP mỗi account — không bao giờ vượt
...
def _state_file(account_id=None):
    """State theo account. account_id=None -> file global (tương thích ngược)."""
    if not account_id:
        return STATE_FILE
    safe = "".join(c for c in str(account_id) if c.isalnum() or c in "-_") or "acc"
    return os.path.join(STATE_DIR, f"rate_{safe}.json")


def _load(account_id=None):
    fp = _state_file(account_id)
...
def _save(st, account_id=None):
    os.makedirs(STATE_DIR, exist_ok=True)
    json.dump(st, open(_state_file(account_id), "w"), ensure_ascii=False, indent=2)
...
def check_can_upload(account_id=None):
    """Trả (ok: bool, reason: str, info: dict). KHÔNG ghi state."""
    st = _rollover(_load(account_id))
    now = _now_ts()
    count = st["count"]
    ups = st["uploads"]
...
def record_upload(video_id=None, title=None, account_id=None):
    """Ghi nhận 1 upload thành công (theo account nếu có)."""
    st = _rollover(_load(account_id))
