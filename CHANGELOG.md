...
### Added — Multi-account rotation + proxy scaffolding (verified LIVE, 2 real channels)
- **accounts.py**: account registry (`state/accounts.json`) with id/email/authuser/channel/proxy/enabled; rotation (round_robin + least_used) that skips accounts at daily cap; `pick_next`, `add/remove/update/set_proxy`, `summary`.
- **Per-account rate limits**: `limits.py` now account-aware (`state/rate_<id>.json`), each account its own 50/day HARD CAP; global legacy state preserved. 2 accounts = 100 videos/day.
- **Proxy scaffolding**: per-account http/socks5 proxy (server+user+pass), validated; uploader reads it; `set-proxy`/`add-account --proxy`.
- **CLI**: 6 new commands (`accounts`, `account-status`, `add-account`, `remove-account`, `set-proxy`, `rotate-pick`) + `--account`/`--rotate` on `upload` + `--account` on list/info/edit/delete/comments/reply/stats/report.
- 12 new tests (34 total).

### Changed
- **session.py rewritten FIFO→CDP**: drives the live headful Chrome via CDP (port 18801) instead of the crash-prone FIFO controller; same API + `set_authuser()`. Stable, no SIGTRAP, no Google "browser not secure" block.
- **Login keeper replaced**: `yt-gui-keeper.sh` (systemd yt-forever) runs Xvfb + real Chrome (CDP + noVNC over Tailscale) keeping the YouTube tab alive forever, auto-restart. Manual login via noVNC (Google blocks headless password login).
- **Uploader**: connects via CDP (new tab) instead of `systemctl stop` + headful relaunch — keeps the persistent tab alive; account switch via `?authuser=N`; forces `?hl=en`.

### Fixed (bugs found during live multi-account test)
- Uploader clobbering the keeper + crashing headful → now CDP-based, tab-preserving.
- English-only selectors failing on Vietnamese-UI account ("Tạo"/"Xóa") → bilingual selectors (Create/Tạo, Delete/Xóa, Delete forever/Xóa vĩnh viễn) + hl=en on manager URLs. Real upload + real delete verified on tanso_nacha channel.
